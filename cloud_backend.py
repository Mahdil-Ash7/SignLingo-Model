...
05_cloud_server.py
==================
Full-pipeline BIM Sign Language backend for Render deployment.
Identical pipeline to 04_fastapi_server.py (local) — Flutter sends JPEG,
server runs MediaPipe + model, returns prediction.

Tuned for Render free tier (512MB RAM / 0.1 CPU):
  - tensorflow-cpu instead of full tensorflow (~120MB saved)
  - MediaPipe model_complexity=0 (lightest model, ~40MB saved)
  - TF threads set to 1 (avoid CPU thrashing on 0.1 CPU)
  - Queue-based worker (single thread, no concurrency issues)
  - Health endpoint returns mode='local' so Flutter treats it same as local

RAM estimate:
  tensorflow-cpu + model : ~200MB
  mediapipe (complexity=0): ~150MB
  FastAPI + Python        : ~80MB
  Total                  : ~430MB  ← within 512MB

Deploy on Render:
  Build command : pip install -r requirements_render.txt
  Start command : uvicorn 05_cloud_server:app --host 0.0.0.0 --port $PORT
  Instance type : Free (512MB / 0.1 CPU)

Flutter config:
  Set kCloudBackendUrl = 'https://your-app.onrender.com'
  No other changes needed — same /predict API as local server.
"""
...
# Render free tier: 0.1 CPU — use 1 thread to avoid context switching overhead
...
holistic = mp_holistic.Holistic(
    ...
    model_complexity=0    # lightest model — saves ~40MB RAM vs complexity=1
)
...
class HealthResponse(BaseModel):
    ...
    smoothing       : int
    mode            : str   # 'local' = full pipeline (same API as local server)
...
def health():
    return HealthResponse(
        ...
        smoothing      =SMOOTHING,
        mode           ="local",   # tells Flutter to use JPEG pipeline, not keypoints
    )
...
if __name__ == "__main__":
    ...
    print(f"\n🚀 BIM Sign Language Cloud API (full pipeline)")
